...
int main(int argc, char** argv) {
    ...
    while ((c = getopt(argc, argv, "ht:u:p:b:m:s:n:l:")) != -1)
        ...
        switch (c) {
            case 'h':
                printf("Usage: [%s] [-t topic[%s]] [-u userid[%s]]  [-p password[%s]]  [-b broker_uri[%s]] [-s message_size[%u]] [-m messages_to_send[%llu]] [-n num_partitions[%u]] [-l loglevel[event]]\n",
    ...
    if (topic == NULL) {
        printf("Usage: [%s] -t topic [-u userid[%s]]  [-p password[%s]]  [-b broker_uri[%s]] [-m message_size[%u]] [-s messages_to_send[%llu]] [-n num_partitions[%u]] [-l loglevel[event]]\n",
                argv[0], userid, password, broker_uri, message_size, messages_to_send, num_partitions);
        return 0;
    }

    printf(" [%s] [-t topic[%s]] [-u userid[%s]]  [-p password[%s]]  [-b broker_uri[%s]] [-s message_size[%u]] [-m messages_to_send[%llu]] [-n num_partitions[%u]] [-l loglevel[%s]]\n",
